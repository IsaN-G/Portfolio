...
const skills = [
  ...
  { title: "Problem Solving & Clean Code", icon: <Terminal size={28} />, description: "Refactoring, Debugging, Performance Optimierung", level: 50, color: "from-blue-500 to-sky-600" },
];
    ...
    <section className="py-16 md:py-24 bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-950">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {skills.map((skill) => (
            <div key={skill.title} className={`group relative bg-white dark:bg-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2`}>
              <div className={`absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r ${skill.color} rounded-t-2xl animate-growWidth`} />
              <div className="flex items-center gap-4 mb-5">
                <div className={`p-3 rounded-xl bg-gradient-to-br ${skill.color} text-white`}>
                  {skill.icon}
                </div>
                <h3 className="text-xl md:text-2xl font-semibold text-gray-900 dark:text-white">{skill.title}</h3>
              </div>
              <p className="text-gray-600 dark:text-gray-300 mb-6">{skill.description}</p>
              <div className="relative pt-1">
                <div className="overflow-hidden h-2 mb-2 text-xs flex rounded bg-gray-200 dark:bg-slate-700">
                  <div style={{ width: `${skill.level}%` }} className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-gradient-to-r ${skill.color} transition-all duration-1000 ease-out`} />
                </div>
